...
本格開発に入る前に、以下を1週間で検証する:

1. **日本語テキスト判定の精度**: 公開モデルが日本語でどの程度使えるか
2. **ファインチューニングの効果**: 日本語データで追加学習した場合の精度向上幅
3. **推論速度**: 実用的なレスポンスタイム（<5秒）を実現できるか
4. **段階的返却**: 統計分析→ML判定の2段階パイプラインが実現できるか
...
- 公開モデル: roberta-base-openai-detector, Hello-SimpleAI/chatgpt-detector-roberta
- 日本語モデル: tohoku-nlp/bert-large-japanese-v2 をベースにファインチューニング
- テストデータ: GPT-4o / Claude 3.5 で生成した日本語テキスト 1000件 + 人間テキスト 1000件
...
| 指標 | 目標値 | 備考 |
|------|--------|------|
| 英語テキスト F1-score | >= 0.90 | 公開モデルで達成見込み |
| 日本語テキスト F1-score | >= 0.80 | ファインチューニング後 |
| 偽陽性率 (FPR) | <= 0.10 | 人間テキストをAI判定する割合 |
| 推論時間 (GPU) | <= 3秒/テキスト | DeBERTa-v3-large、1000文字のテキスト |
| 推論時間 (CPU) | <= 10秒/テキスト | DistilBERT、1000文字のテキスト |

### 8.4 PoCの結果に応じた判断

- **基準達成**: MVP開発に着手
- **日本語精度のみ未達**: データ収集を強化し、2週間の追加PoC
- **全体的に精度不足**: アプローチの見直し（外部APIの利用検討、判定対象の絞り込み）
...
| 脅威 | 対策 |
|------|------|
| A01: アクセス制御の不備 | RBAC（Role-Based Access Control）、APIキーによる認証 |
| A02: 暗号化の失敗 | HTTPS強制、パスワードのbcryptハッシュ化、機密データのAES-256暗号化 |
| A03: インジェクション | SQLAlchemy ORM使用（SQLインジェクション防止）、入力バリデーション |
| A04: 安全でない設計 | 脅威モデリングの実施、入力サイズ制限 |
| A05: セキュリティ設定ミス | IaCによる一貫した設定、CIS Benchmarkの適用 |
| A06: 脆弱なコンポーネント | Dependabotによる自動脆弱性スキャン、定期的な依存更新 |
| A07: 認証の失敗 | JWT有効期限設定(15分)、リフレッシュトークン、ブルートフォース防止 |
| A08: ソフトウェアとデータの整合性の不備 | CI/CDパイプラインの保護、依存関係のハッシュ検証 |
| A09: ログとモニタリングの不備 | 構造化ログ、異常検知アラート、監査ログの保持 |
| A10: SSRF | アップロードファイルの内容検証、外部リクエストの制限 |

### 9.2 入力セキュリティ

- テキスト入力: 最大10万文字。HTMLタグ・スクリプトのサニタイズ
- ファイルアップロード: 最大10MB。MIME type検証、マジックバイト検証、ウイルススキャン（ClamAV）
- APIレート制限: IP単位 + APIキー単位の二重制限

---

## 10. 次のアクション

### 10.1 開発部として即座に着手すべきこと

1. **PoC実施（最優先）**: テキスト判定エンジンの概念実証を1週間で実施
2. **データ収集パイプラインの構築**: GPT-4o, Claude, Gemini等のAPIを利用した日本語テキスト生成・収集
3. **開発環境の構築**: Docker Compose で開発環境を一発起動できるようにする
4. **OpenAPI仕様の策定**: フロントエンド・外部連携を見据えたAPI設計

### 10.2 他部署との連携事項

| 連携先 | 依頼内容 | 優先度 |
|--------|----------|--------|
| PM室 | 開発スケジュール・マイルストーンの策定、PoC完了後のGo/No-Go判断 | 高 |
| デザイン部 | 判定結果の可視化UI/UXデザイン（ヒートマップ、スコア表示のデザイン） | 高 |
| 法務部 | データ取り扱いポリシー策定、利用規約策定、AI出力の著作権確認 | 高 |
| マーケティング部 | ターゲットユーザーの明確化、競合分析、プライシング戦略 | 中 |

### 10.3 技術的な意思決定を要する事項（社長判断待ち）

1. **プロダクト名の決定**: 仮称「DetectAI」または「AI鑑定士」
2. **初期ターゲット**: 教育機関向け / 企業向け / 個人向け のどれを優先するか（技術設計に影響）
3. **料金体系**: Free / Pro / Enterprise の機能差（API制限、バッチ処理の有無等）
4. **データ保持方針**: 判定テキストの保存期間（プライバシーとモデル改善のトレードオフ）
